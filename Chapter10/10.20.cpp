...
void biggies(vector<string> &vec, vector<string>::size_type sz) {
	...
	vector<string>::iterator firstCorrect = stable_partition(vec.begin(), vec.end(), [sz](const string &s) -> bool { return (s.size() < sz) ? true : false; });
	
	// get count of elements that have a minimum of sz size
	vector<string>::difference_type count = count_if(vec.cbegin(), vec.cend(), [sz](const string &s) {return (s.size() >= sz);});
